...
const AI_TOOLS = [
  ...
  { name: 'get_weather', description: 'Hava durumu bilgisini getirir.', input_schema: { type: 'object', properties: {}, required: [] } },
...
function executeTool(name, input, store) {
  ...
  try {
    switch (name) {
      ...
      case 'get_weather': { const wxCities = JSON.parse(localStorage.getItem('gn_wx_cities') || '[]'); if (!wxCities.length) return 'Hava durumu için şehir kaydedilmemiş.'; return JSON.stringify({ city: wxCities[0].name }); }
...
export default function Ai() {
  ...
  const SUGGESTIONS = ['Bugün planım ne?', 'Bu hafta hedeflerim?', 'Trabzon hava durumu?', 'Son filmlerim neler?', 'Güncel haberler?'];
      ...
      <div ref={messagesRef} className="flex-1 overflow-y-auto pr-1" style={{ scrollbarWidth: 'thin' }}>
        {messages.length === 0 && (
          <div className="py-12 text-center animate-fadeIn">
            ...
            <div className="text-xs text-muted2 mb-5">Takvim, hedefler, internet araştırması ve daha fazlası.</div>
      ...
      <div className="mt-3 bg-surface border border-border rounded-2xl p-1">
        <div className="flex items-end gap-2 px-3 py-2">
          <textarea
            ...
            placeholder="Bir şey sorun... (mikrofona bas: sesli konuş)"
            rows={1}
          ...
          />
          <button onClick={listening ? stopListen : startListen} title={listening ? 'Durdur' : 'Sesli konuş'} style={{ width:34, height:34, borderRadius:10, border:'none', flexShrink:0, background: listening ? 'rgba(239,68,68,.2)' : 'rgba(255,255,255,.08)', color: listening ? '#f87171' : 'rgba(232,237,245,.5)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s', animation: listening ? 'pulse 1s ease-in-out infinite' : 'none' }}>
          ...
          <button onClick={() => send()} disabled={loading || !input.trim()} style={{ width:34, height:34, borderRadius:10, border:'none', background: input.trim() && !loading ? '#3a7bd5' : 'rgba(255,255,255,.08)', color:'#fff', cursor: input.trim() && !loading ? 'pointer' : 'default', display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s', flexShrink:0 }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">

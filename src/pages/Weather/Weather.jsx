...
export default function Weather() {
  const { db, setDb, wxCities, setWxCities } = useStore();
  ...
  const saveCities = (list) => {
    setWxCities(list); // localStorage + Firestore senkron (fsWrite) + reaktif state — useStore.js
  };

  // ── Şehir sırası sürükle-bırak ──────────────────────────────────
  // Soldaki (index 0) şehir widget'larda gösterilen şehirdir.
  ...
  const handleDrop = (i) => {
    ...
    saveCities(list);
    const newActiveIdx = list.findIndex(c => c.lat === activeCityObj?.lat && c.lon === activeCityObj?.lon);
    setActiveCity(newActiveIdx >= 0 ? newActiveIdx : 0);
  ...
  const fetchWeather = async (city) => {
    ...
    try {
      ...
      const airRes = await fetch(airUrl).catch(() => null);
      const airJson = airRes?.ok ? await airRes.json() : null;
      const res = await fetch(url);
      if (!res.ok) throw new Error(`Hava durumu servisi şu an yanıt vermiyor (${res.status})`);
      const json = await res.json();
      if (!json.current || !json.daily || !json.hourly) throw new Error('Hava durumu verisi eksik geldi');
      setData({ ...json, city, air: airJson });

      // Akıllı bilgi şeridi için hava durumu özetini Firestore'a yaz (db.wx)
      // Firestore undefined değer kabul etmiyor, bu yüzden eksik alanları atlıyoruz.
      const c = json.current;
      if (c.temperature_2m != null) {
        setDb({ ...db, wx: { temp: c.temperature_2m, uv: c.uv_index ?? null, code: c.weather_code ?? null, city: city.name, ts: Date.now() } });
      }
    } catch(e) {
      console.error(e);
      setFetchError(e.message || 'Hava durumu yüklenemedi');
      setRetryAt(Date.now());
    }
    setLoading(false);
  };

  // Hata olduğunda otomatik olarak birkaç saniye sonra yeniden dener — kullanıcı müdahalesi gerekmez.
  useEffect(() => {
    if (!fetchError) return;
    const t = setTimeout(() => {
      if (cities[activeCity]) fetchWeather(cities[activeCity]);
    }, 8000);
    ...
    <div className="animate-fadeIn" style={{ minHeight:'100vh', display:'flex', flexDirection:'column', padding:'20px 24px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16, flexWrap:'wrap' }}>
        <div style={{ display:'flex', gap:4, flex:1, overflowX:'auto', scrollbarWidth:'none' }}>
          <style>{`
            @keyframes wxPrimaryDot { 0%,100%{opacity:.5;transform:scale(1)} 50%{opacity:1;transform:scale(1.25)} }
          `}</style>
          {cities.map((c, i) => (
            <button key={`${c.lat}-${c.lon}`}
              draggable
              ...
              title={i === 0 ? 'Widget\'ta gösterilen şehir' : 'Sıralamak için sürükle'}
              style={{
                padding:'5px 14px', borderRadius:20,
                border:`1px solid ${overIdx===i && dragIdx!==null && dragIdx!==i ? 'rgba(58,123,213,.9)' : (i===activeCity?'rgba(58,123,213,.5)':'rgba(255,255,255,.08)')}`,
                background: i===activeCity?'rgba(58,123,213,.12)':'transparent',
                color: i===activeCity?'#e8edf5':'rgba(232,237,245,.5)',
                fontSize:12, whiteSpace:'nowrap', display:'flex', alignItems:'center', gap:6,
                cursor: dragIdx===i ? 'grabbing' : 'grab',
                opacity: dragIdx===i ? .35 : 1,
                transform: overIdx===i && dragIdx!==null && dragIdx!==i ? 'scale(1.06)' : 'scale(1)',
                transition:'transform .15s ease, border-color .15s ease, opacity .15s ease',
              }}>
              {i === 0 && (
                <span style={{ width:5, height:5, borderRadius:'50%', background:'#3a7bd5', display:'inline-block', animation:'wxPrimaryDot 1.8s ease-in-out infinite' }}/>
              )}
              {c.name}
      ...
      {/* Hata — otomatik olarak yeniden dener, kullanıcı müdahalesi gerekmez */}
      {fetchError && !loading && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10, padding:'40px 0', textAlign:'center' }}>
          <div style={{ width:16, height:16, border:'2px solid rgba(255,255,255,.1)', borderTopColor:'#3a7bd5', borderRadius:'50%', animation:'spin .8s linear infinite' }} />
          <div style={{ fontSize:13, color:'rgba(232,237,245,.5)' }}>{fetchError}</div>
          <div style={{ fontSize:11, color:'rgba(232,237,245,.3)' }}>Otomatik olarak yeniden denenecek</div>
        </div>
      )}
